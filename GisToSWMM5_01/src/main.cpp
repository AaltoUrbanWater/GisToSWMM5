...
int main (int argc, char* cArgv[])
{
	...
	if (argc == 17 || argc == 18)
	{
			...
			resReport == 0 && resSymbols == 0 && resSnowpacks == 0)
		{
			if (argc == 17)
			{
				...
				grid.saveRaster(cArgv[15]);

				// Create the SWMM5 file.
				std::cout << "\n\nCreating the SWMM5 model input file:";
				grid.saveSWMM5File(headerTable, evaporationTable, temperatureTable, reportTable, snowpacksTable, raingagesTable, symbolsTable, juncTable, outfallsTable, condTable, xsectionTable, cArgv[16]);
		...
		else
		{
			...
			// TJN 15 Mar 2017 START Add this
            if (resXsections > 0)
            {
	...
	{
		std::cout << "\nError, number of command line arguments should be 17 or 18.";
	}
