...
void Grid::computeCellSlopes()
{
	for (int i = 0; i < nCols * nRows; i++)
	{
		...
		for (int k = 0; k < 8; k++)
		{
			...
			{
				// Compute slope.
				if (cells[i].cellSize > 0.0)
				{
					double distance = cells[i].cellSize;
...
void Grid::saveSWMM5File(Table &headerTable, Table &evaporationTable, Table &temperatureTable, Table &reportTable, Table &snowpacksTable, Table &raingagesTable, Table &symbolsTable, Table &juncTable, Table &outfallsTable, Table &condTable, std::string path)
{
	std::stringstream sstream;
	sstream << std::fixed;
	...
	for (int i = 0; i < headerTable.nRows; i++)
	{
		if (i > 0)
		{
			sstream << "\n";
	...
	for (int i = 0; i < evaporationTable.nRows; i++)
	{
		if (i > 0)
		{
			sstream << "\n";
		...
		sstream << evaporationTable.data[i];
	}
	...
	for (int i = 0; i < temperatureTable.nRows; i++)
	{
		if (i > 0)
		{
			sstream << "\n";
		...
		sstream << temperatureTable.data[i];
	}
	...
	for (int i = 0; i < raingagesTable.nRows; i++)
	{
		if (i > 0)
		{
			sstream << "\n";
	...
	for (int i = 0; i < snowpacksTable.nRows; i++)
	{
		if (i > 0)
		{
			sstream << "\n";
	...
	for (int i = 1; i < juncTable.nRows; i++)
	{
		...
		sstream << juncTable.data[i * juncTable.nCols + 4] << "   ";
		sstream << juncTable.data[i * juncTable.nCols + 5] << "   ";
		sstream << 0.0 << "   ";
		sstream << 0.0 << "   ";
		sstream << 0.0 << "   ";
	}
	...
	for (int i = 1; i < condTable.nRows; i++)
	{
		...
		sstream << condTable.data[i * condTable.nCols + 10] << "   ";
		sstream << condTable.data[i * condTable.nCols + 8] << "   ";
		sstream << condTable.data[i * condTable.nCols + 11] << "   "; // roughness
		sstream << condTable.data[i * condTable.nCols + 12] << "   ";
		sstream << condTable.data[i * condTable.nCols + 13] << "   ";
		sstream << 0.0 << "   ";
		sstream << 0.0 << "   ";
	}
	...
	for (int i = 0; i < reportTable.nRows; i++)
	{
		if (i > 0)
		{
			sstream << "\n";
	...
	for (int i = 0; i < symbolsTable.nRows; i++)
	{
		if (i > 0)
		{
			sstream << "\n";
...
void Grid::printReport(Table &catchPropTable)
{
	...
	if (nCols * nRows > 0)
	{
		elevationAverage /= (double)(nCols * nRows);
		slopeAverage /= (double)(nCols * nRows);
	}

	std::cout << "\n-> Total number of cells: " << nCols * nRows;
	std::cout << "\n-> Number of active cells: " << numOfActiveCells;
	std::cout << "\n-> Catchment area: " << (double)numOfActiveCells * cellSize * cellSize / 10000.0 << " ha";
	std::cout << "\n-> Catchment average elevation: " << elevationAverage << " m";
	std::cout << "\n-> Catchment average slope: " << slopeAverage << " m";
	...
	for (int i = 0; i < (int)cellsInlandUseClass.size(); i++)
	{
		...
		<< "\t" << cellsInlandUseClass[i] * cellSize * cellSize / 10000.0
		<< "\t" << (double)cellsInlandUseClass[i] / (double)numOfActiveCells * 100.0;
	}
}